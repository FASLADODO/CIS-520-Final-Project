...
root = split_node(X, Z, Xrange, sum(Z)./sum(Z(:)), 1:size(Xrange, 2), 0, depth_limit);
...
if depth == depth_limit || (numel(unique(Z( :,n))==1) || numel(Z,1) <= 1 || numel(colidx) == 0)
    node.terminal = true;
    node.fidx = [];
    node.fval = [];
    if numel(Z) == 0
...
[node.fidx node.fval max_ig] = dt_choose_feature(X, Z, Xrange, colidx);
